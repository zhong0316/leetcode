...
//        String[] words = {"a", "b", "ab", "abc"};
    ...
    public List<String> findAllConcatenatedWordsInADict(String[] words) {
        ...
        for (String word : words) {
            if (word == null || word.length() == 0) continue;
            if (dfs(word, 0, root)) res.add(word);
        }
        return res;
    }

    private boolean dfs(String word, int counter, TrieNode root) {
        if (counter > 0 && searchTrie(word, root)) {
            return true;
        }
        for (int i = 1; i < word.length(); i++) {
            String leftPart = word.substring(0, i);
            String rightPart = word.substring(i);
            if (searchTrie(leftPart, root)) {
                counter++;
                if (dfs(rightPart, counter, root)) {
                    return true;
                }
            }
        }
    ...
    private boolean searchTrie(String word, TrieNode root) {
        for (int i = 0; i < word.length(); i++) {
            ...
            TrieNode child = root.children[word.charAt(i) - 'a'];
            if (child == null) return false;
            if (i == word.length() - 1 && child.isWord) return true;
            root = child;
        }
        return false;
    }

    private void insertNode(String word, TrieNode root) {
        for (int i = 0; i < word.length(); i++) {
            if (root.children == null) root.children = new TrieNode[26];
            if (root.children[word.charAt(i) - 'a'] == null) root.children[word.charAt(i) - 'a'] = new TrieNode();
            TrieNode child = root.children[word.charAt(i) - 'a'];
